  ...
  return (
    ...
    >
      ...
      <Form.Item
        ...
        name="email"
        rules={[
          { required: true, message: ()=>t('Email is required'.toLowerCase().replace("_")) },
          { type: 'email', message: () => t("Doesn't look like a valid email".toLowerCase().replace(/ /g,"_"))},
        ]}
      >
  ...
  return (
    <Page maxWidth={600}>
      <AuthenticationWrapper className={className}>
        <AuthenticationHeader>{(t("Request verification email".toLowerCase().replace(/ /g,"_")))}</AuthenticationHeader>
